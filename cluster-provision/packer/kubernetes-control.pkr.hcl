...
# Build configuration
source "amazon-ebs" "kubernetes_control" {
  ami_name      = "kubernetes-control-${var.kubernetes_version}-${formatdate("YYYY-MM-DD-hhmm", timestamp())}"
  ...
  # Tags for the AMI
  tags = {
    Name                = "kubernetes-control-${var.kubernetes_version}"
    Type                = "control-plane"
  ...
  run_tags = {
    Name = "packer-kubernetes-control-builder"
    Type = "temporary"
  }
}
...
build {
  ...
  sources = ["source.amazon-ebs.kubernetes_control"]
  ...
  # Copy install script
  provisioner "file" {
    source      = "scripts/install-kubernetes-control.sh"
    destination = "/tmp/install-kubernetes-control.sh"
  }
  
  # Install base Kubernetes components + control plane tools
  provisioner "shell" {
    inline = [
      "export KUBERNETES_VERSION='${var.kubernetes_version}'",
      "export CONTAINERD_VERSION='${var.containerd_version}'",
      "sudo -E bash /tmp/install-kubernetes-control.sh"
    ]
  }
